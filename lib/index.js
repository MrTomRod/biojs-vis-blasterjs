...
module.exports = blasterjs = Class(/** @lends Biojs.blasterjs# */
    {
        ...
        _displayAlignments: function (evt, self) {
                    ...
                    if (lines[i].startsWith('>')) {
                        ...
                        do {
                            ...
                            if (lines[0].startsWith('BLASTN')) {
                                positives = 'N/A';
                                gaps = lines[currentLine + 3].split(',')[1].split('(')[1].substr(0, lines[currentLine + 3].split(',')[1].split('(')[1].length - 2);
                            } else {
                                positives = lines[currentLine + 3].split(',')[1].split('(')[1].substr(0, lines[currentLine + 3].split(',')[1].split('(')[1].length - 2);
                                gaps = lines[currentLine + 3].split(',')[2].split('(')[1].substr(0, lines[currentLine + 3].split(',')[2].split('(')[1].length - 2);
                            }
                            if (lines[currentLine + 4].split(',')[0].split(' ')[1] === 'Frame' || lines[currentLine + 4].startsWith(' Strand')) {
                                currentLine = currentLine + 1;
                            }
                            const queryStart = lines[currentLine + 5].substring(5).replace(/^\s+/g, '').split(' ')[0];
                            let query = lines[currentLine + 5].substring(5).replace(/\s+/g, '').replace(/[0-9]/g, '');
                            let queryEnd = lines[currentLine + 5].substring(5).replace(/^\s+/g, '').split(' ')[lines[currentLine + 5].substring(5).replace(/^\s+/g, '').split(' ').length - 1];
                            let comparison = lines[currentLine + 6].replace(/^\s+/g, '');
                            const sbjctStart = lines[currentLine + 7].substring(5).replace(/^\s+/g, '').split(' ')[0];
                            let sbjct = lines[currentLine + 7].substring(5).replace(/\s+/g, '').replace(/[0-9]/g, '');
                            let sbjctEnd = lines[currentLine + 7].substring(5).replace(/^\s+/g, '').split(' ')[lines[currentLine + 7].substring(5).replace(/^\s+/g, '').split(' ').length - 1];

                            currentLine = currentLine + 9;
                            while (lines[currentLine].startsWith('Query')) {
                                const nextQuery = lines[currentLine].substring(5).replace(/\s+/g, '').replace(/[0-9]/g, '');
                                query += nextQuery;
                                ...
                                let nextComparison = lines[currentLine + 1].replace(/^\s+/g, '');
                                if (nextQuery.length > nextComparison.length) {
                                    const diference = nextQuery.length - nextComparison.length;
                                    for (let j = 0; j < diference; j++) {
                            ...
                            const hsp = new HSP(score, eValue, identities, positives, gaps, queryStart, query, queryEnd, comparison, sbjctStart, sbjct, sbjctEnd);
                            hsps.push(hsp);
                            multiple = true;
            ...
            function createChangeScoringButton(queries, hsps, position, alignments, lenght, colored, scoring, self) {
                ...
                const text = document.createTextNode(scoring ? 'Change scoring to E value' : 'Change scoring to Max score')
            ...
            function createChangeColorButton(queries, hsps, position, alignments, lenght, colored, scoring, self) {
                ...
                button.style.marginRight = '10px';
                button.style.marginTop = '5px';
                const text = document.createTextNode(colored ? 'Change colours to grayscale' : 'Change colours to full colored')
                button.appendChild(text);
                button.onclick = function () {
            ...
            function createDownloadButton(type) {
                ...
                button.style.marginRight = '10px';
                button.style.marginTop = '5px';
                const text = document.createTextNode('Download as ' + type);
                button.appendChild(text);
            ...
            function downloadAlignmentsImg(type) {
                const buttons = document.getElementById('blast-multiple-alignments-buttons');
                const input = document.getElementById('defline');
                const container = document.getElementById('alignments-container');
            ...
            function createAlignmentComparison(queries, hsps, position, alignments, queryLength, colored, scoring, self) {
                ...
                const buttonDownloadJPEG = createDownloadButton('JPEG');
                blastDiv.style.paddingTop = '20px';
                input.id = 'defline';
                input.name = 'defline';
                input.type = 'text';
            ...
            function createTableButtons(alignments) {
                ...
                const butImgJPEG = document.createElement('button');
                const butImgPNG = document.createElement('button');
                ...
                butImgPNG.onclick = function () {
                    downloadTableImg('PNG');
                };
                butImgPNG.style.marginRight = '10px';
                butImgJPEG.className = 'btn';
                butImgJPEG.innerHTML = 'Download as JPEG';
                butImgJPEG.onclick = function () {
                    downloadTableImg('JPEG');
                };
            ...
            function downloadTableImg(type) {
                ...
                const container = document.getElementById('blast-alignments-table-img');
                html2canvas(container, {
                    onrendered: function (canvas) {
                        document.body.appendChild(canvas);
                        let a = document.createElement('a');
            ...
            function createSingleAlignment(alignment, self, hsps, hsp) {
                ...
                const alignmentContainer = document.createElement('div');
                const buttonDownloadJPEG = document.createElement('button');
            ...
            function downloadSingleAlignmentImg(alignment, type) {
                const container = document.getElementById('blast-single-alignment-pre');
                const button = document.getElementById('blast-single-alignment-next');
                ...
                html2canvas(container, {
                    onrendered: function (canvas) {
                        ...
                        document.body.removeChild(canvas);
                        document.body.removeChild(a);
                        if (typeof (button) != 'undefined' && button != null) container.appendChild(button);
                        a = document.createElement('a');
                        document.body.appendChild(a);
